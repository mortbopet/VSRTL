...
class ComponentButton : public QGraphicsObject {
    Q_OBJECT
public:
    ComponentButton(QGraphicsItem* parentItem = nullptr) : QGraphicsObject(parentItem) { setFlags(ItemIsSelectable); }
    ...
    QRectF boundingRect() const override { return QRectF(0, 0, GRID_SIZE * 2, GRID_SIZE * 2); }
    QPainterPath shape() const override {
        QPainterPath p;
        p.addRect(QRectF(GRID_SIZE / 4, GRID_SIZE / 4, GRID_SIZE * 1.75, GRID_SIZE * 1.75));
        return p;
    }
    ...
    void paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*) override {
        ...
        if (!m_expanded) {
            painter->drawLine(QPointF(rect.width() / 2, GRID_SIZE / 2),
