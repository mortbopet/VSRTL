...
public:
    NetlistView(QWidget* parent) : QTreeView(parent) {
        ...
        connect(this, &QTreeView::customContextMenuRequested, this, &NetlistView<T>::contextMenuRequest);
    }
...
private slots:
    void contextMenuRequest(const QPoint& pos) {
        ...
        if (index.isValid()) {
            /* The TreeItem at the given index will be queried for its available actions (both TreeItem actions and
             * derived class actions). These actions are then presented in the context menu */
