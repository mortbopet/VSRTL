...
class OpMul : public Component {
public:
    SetGraphicsType(Adder);
    OpMul(std::string name, SimComponent* parent, unsigned int w_op1, unsigned int w_op2) : Component(name, parent) {
        DYNP_IN_INIT(op1, w_op1);
        DYNP_IN_INIT(op2, w_op2);
        const unsigned int w_out = w_op1 + w_op2;
