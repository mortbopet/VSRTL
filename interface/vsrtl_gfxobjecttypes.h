...
public:
    ...
    static void registerGraphicsType(const std::string& name, const GraphicsType* obj) {
        auto* instance = _get();
        const auto name_lower = str_toLower(name);
        if (instance->m_nameToType.count(name_lower) != 0) {
            throw std::runtime_error("Graphics type already registerred for type '" + name + "'");
        }
...
#define DefineGraphicsType(type, requiredSpecialPorts)                                            \
    class GraphicsTypeForComponent(type)                                                          \
        : public GraphicsType{public : const std::vector<std::string> specialPortIDs()            \
    ...
    std::string getName() const override { return #type; }                                        \
                                                                                                  \
private:                                                                                          \
    GraphicsTypeForComponent(type)() { GraphicsTypeFromName::registerGraphicsType(#type, this); } \
    }                                                                                             \
    ;                                                                                             \
    static auto* __##type##gfxtypeinstance__ = GraphicsTypeFor(type);  // Static initializer to register type on startup
