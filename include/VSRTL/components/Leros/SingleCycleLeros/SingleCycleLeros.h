...
public:
  ...
  SUBCOMPONENT(regs,
               TYPE(MemoryAsyncRd<ceillog2(LEROS_REGS), LEROS_REG_WIDTH>));

  // Multiplexers
  SUBCOMPONENT(reg_out_data_mux,
               TYPE(EnumMultiplexer<reg_data_src, LEROS_REG_WIDTH>));
  SUBCOMPONENT(mem_out_data_mux,
               TYPE(EnumMultiplexer<mem_op, LEROS_REG_WIDTH>));
  SUBCOMPONENT(mem_out_addr_mux,
               TYPE(EnumMultiplexer<mem_op, LEROS_REG_WIDTH>));
  ...
  SUBCOMPONENT(pc_reg_next_mux,
               TYPE(EnumMultiplexer<pc_reg_src, LEROS_REG_WIDTH>));
  SUBCOMPONENT(acc_reg_next_mux,
               TYPE(EnumMultiplexer<acc_reg_src, LEROS_REG_WIDTH>));
  SUBCOMPONENT(addr_reg_next_mux,
               TYPE(EnumMultiplexer<addr_reg_src, LEROS_REG_WIDTH>));

  // Comparators
  SUBCOMPONENT(dm_wr_en, Eq<enumBitWidth<mem_op>()>);
  SUBCOMPONENT(reg_wr_en, Eq<enumBitWidth<mem_op>()>);
